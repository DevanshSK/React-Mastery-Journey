    ...
    return (
        <>
            <div className="relative w-[50%] rounded-lg overflow-hidden">
                <motion.div
                    className={`flex mb-[2rem] transition-opacity duration-300 ${true || showThumbnails || isHovered ? "opacity-100" : "opacity-0"}`}
                >
                    {images.map((image, index) => (
                        <div
                            ...
                            className={`w-[10rem] h-[5rem] mt-[2rem] cursor-pointer rounded-lg overflow-hidden border-2 ${currentIndex === index ? "border-blue-500" : "border-transparent"}`}
                        >
                ...
                >
                    ...
                    <button
                        onClick={prevSlide}
                        className={`carousel-button absolute left-4 transform
             -translate-y-1/2 p-2 shadow transition-opacity duration-300 ${isHovered ? "opacity-100" : "opacity-0"
                            }`}
                    >
                        &#10094;
                    </button>
                    <button
                        onClick={nextSlide}
                        className={`carousel-button absolute right-4 transform 
            -translate-y-1/2 p-2 shadow transition-opacity duration-300 ${isHovered ? "opacity-100" : "opacity-0"
                            }`}
                    >
                        &#10095;
